...
TOP_MODULE = hub75_controller_top
TESTBENCH = hub75_tb

# Archivos fuente del sistema
SOURCES = \
    hub75_controller_top.v \
    panel_fsm.v \
    panel_memory.v \
    scan_counter.v \
    delay_unit.v
...
# Configuración FPGA (Colorlight 5A-75E v8.2)
DEVICE = 25k
PACKAGE = CABGA256
SPEED = 7
...
.PHONY: all clean sim view synth pnr bitstream program dirs test stats timing help
...
sim: dirs $(BUILD_DIR)/image.hex
	@echo "$(COLOR_BLUE)Compilando testbench...$(COLOR_RESET)"
	$(IVERILOG) -o $(BUILD_DIR)/$(TESTBENCH).vvp \
	    -s $(TESTBENCH) \
	    $(TESTBENCH).v $(SOURCES) \
	    -DIMAGE_HEX_PATH=\"$(BUILD_DIR)/image.hex\" \
	    -DSIMULATION
...
program: bitstream
	...
	openFPGALoader --cable ft232RL --pins=0:3:4:1 --freq 3000000 \
	    -b colorlight $(BUILD_DIR)/$(PROJECT).bit
...
help:
	...
	@echo "  make all        - Síntesis + PNR + Bitstream"
	@echo "  make sim        - Simulación con Icarus"
	@echo "  make view       - Ver señales en GTKWave"
	@echo "  make synth      - Síntesis con Yosys"
	@echo "  make pnr        - Place & Route"
	@echo "  make bitstream  - Generar bitstream"
	@echo "  make program    - Programar FPGA"
